...
export const getCotacoes = async (req: Request, res: Response) => {
    try {
        const query = `
            SELECT c.cotacao_id, c.destinatario, c.destinatario_sobrenome, c.cep_destino, c.status, c.data_criacao, e.nome_fantasia as empresa_origem
            FROM cotacoes c
...
export const createCotacao = async (req: Request, res: Response) => {
    const { empresa_origem_id, cep_destino, itens, destinatario, destinatario_sobrenome } = req.body as {
    ...
    try {
        ...
        const cotacaoRes = await client.query(
            'INSERT INTO cotacoes (empresa_origem_id, cep_destino, status, destinatario, destinatario_sobrenome) VALUES ($1, $2, $3, $4, $5) RETURNING *',
            [empresa_origem_id, cep_destino, 'PROCESSANDO', destinatario, destinatario_sobrenome]
        );
        ...
        const headers = { 'Accept': 'application/json', 'Authorization': `Bearer ${process.env.MELHOR_ENVIO_TOKEN}`, 'Content-Type': 'application/json', 'User-Agent': `Aplicação ${process.env.ME_EMAIL || 'contato@empresa.com.br'}`};
...
export const reprintLabel = async (req: Request, res: Response) => {
    ...
    try {
        const result = await pool.query('SELECT order_id FROM cotacoes WHERE cotacao_id = $1', [cotacao_id]);
        if (result.rowCount === 0 || !result.rows[0].order_id) {
            return res.status(404).json({ message: 'Etiqueta não encontrada para esta cotação.' });
        }
